...
export const bookingSchema = y
  ...
  .shape({
    ...
    numberOfPositions: y.number().min(1).required(),
    facilitiesRate: y.number().min(1).required(),
    ...
    positionsAvailable: y
      .number()
      .min(1, 'Should be more than one')
      .required('Postions Available is a required field'),
    workingHours: y
      .number()
      .min(1, 'Should be more than one')
    ...
    pricePerHour: y
      .number()
      .min(1, 'Should be more than one')
      .required('Pay per hour is a required field'),
